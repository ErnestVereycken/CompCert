...
(*  | Sifthenelse(e, Sskip, s2) ->
  ...
  | Sswitch(long, e, cases, dfl) ->
    let print_case (n,x) =
      ...
      if long then
        fprintf p "@ (case %Ld LL (Sexit %d))" (Z.to_int64 n) x
      else
        fprintf p "@ (case %ld (Sexit %d))" (Z.to_int32 n) x in
      fprintf p "@[<v 2>(Sswitch%s %a ("
        (if long then "l" else "") print_expr e;
      List.iter print_case cases;
      fprintf p "@ (case default (Sexit %d))" (Nat.to_int dfl);
      fprintf p "))" 
  | Sreturn None ->
      fprintf p "(Sreturn)"
  | Sreturn (Some e) ->
